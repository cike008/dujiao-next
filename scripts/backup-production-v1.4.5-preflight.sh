...
HOST="${1:-dujiao-vps}"
REMOTE_SCRIPT="/tmp/dujiao-next-v1.4.5-preflight-backup.sh"
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "Run: sudo bash /tmp/dujiao-next-v1.4.5-preflight-backup.sh" >&2
  exit 1
fi

TS="$(date +%Y%m%d%H%M%S)"
BACKUP_DIR="/opt/backups/dujiao-next-v1.4.5-preflight-${TS}"
