...
HOST="${1:-dujiao-vps}"
REMOTE_SCRIPT="/tmp/dujiao-next-v1.4.7-production-cutover.sh"
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "Dry run: sudo bash /tmp/dujiao-next-v1.4.7-production-cutover.sh" >&2
  echo "Apply:   sudo bash /tmp/dujiao-next-v1.4.7-production-cutover.sh --apply" >&2
  exit 1
fi
...
TS="$(date +%Y%m%d%H%M%S)"
CUTOVER_DIR="${APP_DIR}/cutover-v1.4.7-${TS}"
...
echo "== v1.4.7 production cutover helper =="
...
if [ "$APPLY" -ne 1 ]; then
  ...
  echo "To apply after review: sudo bash /tmp/dujiao-next-v1.4.7-production-cutover.sh --apply"
  exit 0
fi
