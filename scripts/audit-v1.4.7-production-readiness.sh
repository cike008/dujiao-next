...
HOST="${1:-dujiao-vps}"
REMOTE_SCRIPT="/tmp/dujiao-next-v1.4.7-production-readiness.sh"
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "Run: sudo bash /tmp/dujiao-next-v1.4.7-production-readiness.sh" >&2
  exit 1
fi
...
echo "== Dujiao-Next v1.4.7 Production Readiness Audit =="
...
if "app" not in sections:
    print("FAIL app_section_missing: add persistent app.secret_key before v1.4.7 production cutover")
...
find /opt/backups -maxdepth 1 -type d -name 'dujiao-next-v1.4.7-preflight-*' -printf '%TY-%Tm-%Td %TH:%TM:%TS %p\n' 2>/dev/null | sort | tail -5 || true
