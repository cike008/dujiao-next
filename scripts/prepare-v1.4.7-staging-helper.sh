...
HOST="${1:-dujiao-vps}"
REMOTE_SCRIPT="/tmp/dujiao-next-v1.4.7-staging-restore.sh"
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "Run: sudo bash /tmp/dujiao-next-v1.4.7-staging-restore.sh /opt/backups/dujiao-next-v1.4.7-preflight-YYYYmmddHHMMSS" >&2
  exit 1
fi

BACKUP_DIR="${1:-}"
if [ -z "$BACKUP_DIR" ] || [ ! -f "$BACKUP_DIR/postgres-dujiao.dump" ] || [ ! -f "$BACKUP_DIR/dujiao-next/config.yml" ]; then
  echo "Usage: sudo bash /tmp/dujiao-next-v1.4.7-staging-restore.sh /opt/backups/dujiao-next-v1.4.7-preflight-YYYYmmddHHMMSS" >&2
  echo "The backup directory must contain postgres-dujiao.dump and dujiao-next/config.yml." >&2
  exit 1
fi

STAGE_DIR="/opt/dujiao-next-staging-v1.4.7"
STAGE_DB_PASSWORD="dujiao_staging_only_local"
STAGE_APP_SECRET="$(openssl rand -hex 32)"
IMAGE_TAG="dujiaonext/dujiao-next:teamgenie-v1.4.7"
...
echo "--- starting v1.4.7 TeamGenie staging backend ---"
docker compose up -d dujiaonext-staging

echo "--- staging status ---"
docker compose ps
echo "--- waiting for v1.4.7 backend and migrations ---"
healthy=0
for _ in $(seq 1 60); do
...
if [ "$column_count" != "3" ]; then
  echo "v1.4.7 payment/refund migration columns incomplete: ${column_count}/3" >&2
  exit 1
fi
...
if [ "$marker_count" != "2" ]; then
  echo "v1.4.7 payment/refund migration markers incomplete: ${marker_count}/2" >&2
  exit 1
fi
...
echo "Admin path: http://127.0.0.1:18081/staging-admin-v147/"
...
echo
echo "  sudo bash $REMOTE_SCRIPT /opt/backups/dujiao-next-v1.4.7-preflight-YYYYmmddHHMMSS"
