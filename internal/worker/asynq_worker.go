...
func (c *Consumer) Register(mux *asynq.ServeMux) {
	...
	mux.HandleFunc(queue.TaskTeamGenieSyncFulfilled, c.handleTeamGenieSyncFulfilled)
...
func (c *Consumer) handleTeamGenieSyncFulfilled(_ context.Context, task *asynq.Task) error {
	if c == nil || task == nil {
	...
	if err := c.TeamGenieSyncService.SyncFulfilled(order, order.Fulfillment); err != nil {
